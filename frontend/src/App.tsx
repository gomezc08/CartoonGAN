...
import { pix2Pix, cycleGANImage } from "./lib/api";
...
async function generatecycleGANImage(_file: File) {
  const response = await cycleGANImage(_file);
  // The response already includes the data URL prefix
  return response.cartoonImage;
}
...
export default function CartoonStudio() {
  const [tab, setTab] = useState<"pix_img2img" | "cycle_img2img">("pix_img2img");
  const [results, setResults] = useState<string[]>([]);
  const [loading, setLoading] = useState(false);

  // Separate files per tab
  const [pixFile, setPixFile] = useState<File | null>(null);
  const [cycleFile, setcycleFile] = useState<File | null>(null);
  ...
  const onDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (f && f.type.startsWith("image")) {
      ...
      if (tab === "cycle_img2img") setcycleFile(f);
    }
  };

  const onBrowse = () => {
    if (tab === "pix_img2img") pixFileInputRef.current?.click();
    if (tab === "cycle_img2img") cycleFileInputRef.current?.click();
  };
  ...
  const clearcycleFile = () => {
    setcycleFile(null);
    if (cycleFileInputRef.current) cycleFileInputRef.current.value = "";
  };
  ...
  async function handleGenerate() {
    try {
      ...
      if (tab === "pix_img2img") {
        ...
        url = await generatecycleGANImage(cycleFile);
      }
      setResults((r) => [url, ...r]);
    } catch (e) {
      console.error(e);
    ...
    <div className="min-h-screen w-full bg-zinc-950 text-zinc-100 py-6">
      <header className="mx-auto w-full max-w-7xl px-12 pt-12">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          ...
          />
          <TabBtn
            active={tab === "cycle_img2img"}
            onClick={() => setTab("cycle_img2img")}
            icon={<ImagePlus className="h-4 w-4" />}
            label="cycleGAN"
          />
        </div>
      ...
      <main className="mx-auto mt-12 w-full max-w-7xl px-12">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
          ...
          >
            <div className="mb-5">
              <h2 className="text-lg font-semibold">{tab === "pix_img2img" ? "Pix2Pix (img→img)" : "cycleGAN (img→img)"}</h2>
            </div>
                ...
                >
                  <Field
                    ...
                    right={
                      <div className="flex items-center gap-3">
                        {cycleFile && (
                          <button onClick={clearcycleFile} className="text-xs text-zinc-400 hover:underline">Remove</button>
                        )}
                        <button onClick={onBrowse} className="text-xs text-pink-400 hover:underline">Browse</button>
                      </div>
                    }
                  >
                    ...
                    >
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => setcycleFile(e.target.files?.[0] ?? null)}
                      />
                      {cycleFile ? (
                        <div className="flex w-full items-center gap-4">
                          <img src={URL.createObjectURL(cycleFile)} alt="preview" className="h-20 w-20 rounded-lg object-cover" />
                          <div className="text-left">
                            <p className="text-sm font-medium">{cycleFile.name}</p>
                            <p className="text-xs text-zinc-400">{Math.round(cycleFile.size / 1024)} KB</p>
                          </div>
                        </div>
                      ) : (
                        <div className="flex flex-col items-center gap-2 text-zinc-400">
